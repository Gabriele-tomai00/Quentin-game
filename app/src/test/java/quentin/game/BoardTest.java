...
class BoardTest {
  ...
  void testCorrectPlaceStone() {
    ...
    assertAll(() -> assertEquals(BoardPoint.BLACK, board.getPoint(new Cell(1, 2))),
  ...
  void testCompactString2Tiles() {
    ...
    assertAll(() -> assertEquals(expectedResult, board.toCompactString()),
              () -> assertEquals(expectedResult, board.toCompactString()),
  ...
  void testToAndFromCompactString() {
    for (int i = 0; i < board.size(); i++) { board.placeStone(BoardPoint.BLACK, i, 4); }
