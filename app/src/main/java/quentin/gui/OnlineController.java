...
public class OnlineController implements Initializable {
  ...
  public void displayWinner() {
    Platform.runLater(() -> {
      messageField.setText((winner == game.getCurrentPlayer()
                                          .color() ? "you won" : "you lost").toUpperCase());
    ...
    if (handler.isWaiting()) {
      ...
      try {
        ...
        if ((winner = someoneWon()) != null) {
          handler.sendCommands(CommunicationProtocol.winner(winner == game.getCurrentPlayer()
                                                                          .color() ? "lost" : "won"));
          game.setSomeoneWon();
        }
  ...
  public BoardPoint someoneWon() {
    if (game.hasWon(BoardPoint.BLACK)) { return BoardPoint.BLACK; }
    if (game.hasWon(BoardPoint.WHITE)) { return BoardPoint.WHITE; }
    return null;
  }
