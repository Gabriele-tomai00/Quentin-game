...
public class GuiBoard extends Board {
  ...
  public GuiBoard() {
    ...
    for (int i = 0; i < SIZE; i++) {
      for (int j = 0; j < SIZE; j++) {
        backgroundProperties[i][j] = new SimpleObjectProperty<>(new Background(new BackgroundFill(null,
                                                                                                  CornerRadii.EMPTY,
                                                                                                  Insets.EMPTY)));
      }
    }
  ...
  public void placeStone(BoardPoint stone, int row, int col) {
    ...
    backgroundProperties[row][col].set(new Background(new BackgroundFill(color, CornerRadii.EMPTY, Insets.EMPTY)));
