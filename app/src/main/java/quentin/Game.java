...
public class Game {
  ...
  public boolean hasWon(Player player) {
    if (player.color() == BoardPoint.WHITE) {
      for (Cell cell = new Cell(0, 0); cell.row() < boardSize(); cell.setRow(cell.row() + 1)) {
        if (board.getPoint(cell) == BoardPoint.WHITE && findWinnerPath(BoardPoint.WHITE, cell)) { return true; }
      }
    } else {
      for (Cell cell = new Cell(0, 0); cell.col() < boardSize(); cell.setCol(cell.col() + 1)) {
        if (board.getPoint(cell) == BoardPoint.BLACK && findWinnerPath(BoardPoint.BLACK, cell)) { return true; }
      }
    }
    return false;
  }
  ...
  boolean findWinnerPath(BoardPoint color, Cell startPoint) {
    ...
    while (!toVisit.isEmpty()) {
      ...
      List<Cell> neighbors = getNeighbors(visiting).stream()
  ...
  public void coverTerritories(Cell cell) {
    ...
    for (Cell neighbor : neighbors) {
      ...
      int blacks = (int) frontier.stream()
                                 ...
                                 .count();
      if (whites > blacks) {
        for (Cell tile : territory) { board.placeStone(BoardPoint.WHITE, tile.row(), tile.col()); }
  ...
  public boolean isMoveValid(BoardPoint color, Cell cell) {
    ...
    if (row > 0 && col > 0 && board.getPoint(new Cell(row - 1, col - 1)) == color) {
      if (board.getPoint(new Cell(row, col - 1)) != color && board.getPoint(new Cell(row - 1, col)) != color) {
        return false;
      }
    }
    ...
    if (row < board.size() - 1 && col < board.size() - 1 && board.getPoint(new Cell(row + 1, col + 1)) == color) {
      if (board.getPoint(new Cell(row, col + 1)) != color && board.getPoint(new Cell(row + 1, col)) != color) {
        return false;
      }
    }
  ...
  public int letterToIndex(char letter) {
    ...
    if (upperLetter < 'A' || upperLetter > 'M') { return -1; }
